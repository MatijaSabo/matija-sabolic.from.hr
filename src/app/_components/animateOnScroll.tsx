...
const AnimateOnScroll = ({ children, reappear, threshold = 0.5 }: Props) => {
  ...
  return (
    <>
      <div ref={containerRef} className={`transition duration-1000 ${isVisible ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-1/2"}  motion-reduce:transition-none motion-reduce:hover:transform-none`}>
        {children}
      </div>
    </>
  );
